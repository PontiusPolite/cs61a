...
def prune_min(t):
    ...
    if not t.is_leaf():
        smallest = t.branches[0]
        if len(t.branches) > 1:
            for b in t.branches[1:]:
                if b.label < smallest.label:
                    smallest = b
                    del t.branches[0]
                else:
                    t.branches.remove(b)
...
def num_splits(s, d):
    ...
    12
    """
    # Use a helper to look through subarrays with tree recursion, and add to the total 
    # if those subarrays are within d. The tree is built by moving through the array s,
    # and deciding whether or not to include the element at index i
    def helper(s, d, i=0, sub=[]):
        # If we've reached the end of our subsequence tree and the split difference is
        # within d, return 1. If it's larger than d, return 0.
        if i == len(s):
            if abs(sum(s) - sum(sub) * 2) <= d:
                return 1
            else:
                return 0
        # Otherwise, add a new level to the subsequence tree by including or not including
        # the i'th element of s. Return the sum of these.
        else:
            return helper(s, d, i + 1, sub) + helper(s, d, i + 1, sub + [s[i]])
    
    # Need to divide result by 2, since the tree recursion looks at both the 
    # subsequence and it's complement (which are the same split)
    return helper(s, d) // 2
